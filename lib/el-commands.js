...
async function collectEventLogPromises(elPromises) {
  // wait for all the el queries to complete
  const els = await Promise.all(elPromises)

  // collect all the event docs
  const allDocs = []
  for (const el of els) {
    allDocs.push(...el)
  }

  // throw them in a set to remove dups
  const allDocsSet = new Set()
  for (const doc of allDocs) {
    allDocsSet.add(JSON.stringify(doc))
  }

  // return as a sorted array
  return Array.from(allDocsSet)
    .map(e => JSON.parse(e))
    .sort((e1, e2) => 
